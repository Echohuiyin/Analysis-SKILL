...
description: Verify kernel bug reproduction cases by compiling and testing in QEMU. Use when user mentions '复现验证', '复现用例', 'reproduce test', 'kernel test case', '验证复现', or provides patch/scripts to test kernel behavior. This skill acts as a kernel testing expert that validates reproduction cases and reports results back to kernel experts.
---

# Kernel Reproducer Skill
...
- User asks to verify if a kernel bug can be reproduced
- User mentions "复现验证", "复现用例", "reproduce test case"
...
```markdown
# Kernel Bug Reproduction Report
...
## Conclusion
The bug was successfully reproduced using the provided case. The reproduction process is verified and can be used for further debugging.
...
---
Generated by kernel-reproducer skill
```
...
/kernel-reproducer reproduction_case.yaml
...
```bash
/kernel-reproducer --kernel arch/arm64/boot/Image test_script.sh
...
Minimal input:
```bash
/kernel-reproducer "Test JFFS2 panic with panic-on-oops"
...
### Example 1: Complete Reproduction Case
...
```
/kernel-reproducer "Verify patches/ub_fix.patch causes UB crash on arm64"
```
...
```
/kernel-reproducer failing_case.yaml
```
...
- **Output**: Verified result + actionable feedback
...
## Collaboration with kernel-testcase-generator

This skill validates cases from `/kernel-testcase-generator` (kernel expert skill).

### Role Separation

**kernel-testcase-generator**: Constructs cases
- Analyzes vmcore/lock analysis
- Generates reproducer code
- Self-verification (minimal)
- Outputs case.yaml

**This skill (test-validator)**: Validates cases
- Receives case.yaml
- Full kernel compilation (`/kernel-build`)
...
```
/kernel-testcase-generator → generates case.yaml → THIS SKILL validates

If validation FAILS:
    This skill provides structured feedback
    testcase-generator refines case
    This skill re-validates
    Repeat until SUCCESS
```

### Input from testcase-generator

Standard YAML format:
```yaml
case_id: "BUG-001"
source: "vmcore-analysis-YYYY-MM-DD"
test_case_type: kernel_module
patches: [reproducer.patch]
configs: [CONFIG_X=y]
expected_result: "Should trigger X"
timeout: 150
generator_verification: passed
generator_notes: "Self-check passed"
```

### Output Feedback (if FAILED)
...
## Failure Reasons
1. Config Issues: Missing CONFIG_LOCKDEP=y
2. Test Method: Timeout 60s too short (need 150s)
3. Expected Pattern: "deadlock" not found, use "hung task"

## Recommendations
- Add CONFIG_LOCKDEP=y
- Increase timeout to 150s
...
This skill uses:
- `/kernel-build` - Kernel compilation (required)
- `/qemu-test` - QEMU testing (required)

testcase-generator uses:
- `/vmcore-analyzer` - Input (problem analysis)
- `/lock-analyzer` - Input (lock issues)
- `/rag-case-retrieval` - Input (similar cases)
- `/kernel-build` - Self-check only (minimal)

### Integration Example

Complete example in Analysis-SKILL project:
```
examples/kernel-deadlock-scenario/
├── scenario_description.md           # End-to-end walkthrough
├── generated_case/                   # testcase-generator output
│   ├── reproducer.c
│   ├── case.yaml                     # Handoff to this skill
│   └── verification.log
└── validation_outputs/               # This skill output
    └── CACHE-DEADLOCK-001/
...
**From testcase-generator (input side)**:
1. Clear case metadata (case_id, source analysis)
2. Minimal self-verification (don't over-test)
3. Document design rationale
4. Standard YAML format
5. Include expected patterns

**From this skill (validation side)**:
1. Structured feedback categories
2. Specific config values (not generic)
3. Concrete timeout numbers
4. Evidence-based analysis
5. Save all artifacts for generator review

### Integration Documentation

For complete integration guide:
- `docs/kernel-skills-integration.md` - Full workflow documentation
- `examples/kernel-deadlock-scenario/` - End-to-end example

---

**Workflow**: testcase-generator (construct) → test-validator (validate) → Iterate
