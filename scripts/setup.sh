...
# skill 根目录（脚本的上级目录，即 oc-aicrash-skill/）
...
install_mcp_package() {
    ...
    if [[ ! -f "$PYPROJECT" ]]; then
        ...
        error "请确保在 oc-aicrash-skill 项目目录中运行此脚本"
        return 1
    fi
...
install_skill() {
    ...
    mkdir -p "$SKILL_TARGET_DIR/references/phases"
    mkdir -p "$SKILL_TARGET_DIR/references/reference"
    ...
    # 复制 references/phases/ 子目录文件
    local phase_files=(
        "references/phases/mcp_tools_rules.md"
        "references/phases/phase0_setup.md"
        "references/phases/phase4_community_fix.md"
        "references/phases/phase5_mitigation.md"
        "references/phases/phase6_report.md"
    )
    ...
    # 复制 references/reference/ 子目录文件
    local ref_files=(
        "references/reference/crash_commands.md"
        "references/reference/scenario_analysis.md"
        "references/reference/scenario_bug_null_ptr.md"
        "references/reference/scenario_hung_task.md"
        "references/reference/scenario_lockup.md"
        "references/reference/scenario_oom_deadlock.md"
        "references/reference/scenario_sysrq_virsh.md"
    )
